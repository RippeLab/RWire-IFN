...
#' @param refData ("RNA" | "ATAC"), whether RNA or ATAC should be used as reference data set.
...
addGeneIntegrationMatrix <- function (ArchRProj = NULL, refData = "RNA", useMatrix = "GeneScoreMatrix", matrixName = "GeneIntegrationMatrix", 
    ...
    ...) 
{
    ...
    .logDiffTime(paste0("Computing Integration in ", length(blockList), 
        " Integration Blocks!"), tstart, verbose = verbose, logFile = logFile)
    dfAll <- .safelapply(seq_along(blockList), function(i) {
        ...
        transferAnchors <- .retryCatch({
            ...
            if (refData == "RNA"){
                ...
                ...)
            } else if (refData == "ATAC"){
                Seurat::FindTransferAnchors(reference = seuratATAC, query = subRNA, 
                reduction = reduction, features = genesUse, verbose = FALSE, 
                ...)
            }
        }, maxAttempts = 2, logFile = logFile)
        print(transferAnchors)
        print(class(transferAnchors))
        print(dim(transferAnchors))
        ...
        if (refData == "RNA"){
            ...
            transferParams$refdata <- seuratATAC$Group
        }
        ...
        if (refData == "RNA"){
            ...
            transferParams$refdata <- colnames(seuratATAC)
        }
        ...
        if (addToArrow) {
            ...
            transferParams$refdata <- GetAssayData(seuratATAC, assay = "GeneScore", 
                slot = "data")
            }
            gc()
