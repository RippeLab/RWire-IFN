...
#' @param normMethod The name of the column in `cellColData` by which normalization should be performed or 'raw', if raw counts should be visualized. The recommended and default value
...
.groupRegionSumArrows <- function(
  ...
  if(tolower(normMethod) == "readsintss"){
      ...
      groupNormFactors <- rep(1, length(unique(g)))
      names(groupNormFactors) <- unique(g)
  }else{
