...
#' @param
#' @param aggregation_k The number of k-nearest neighbors to use for creating single-cell groups for correlation analyses (number of cells per aggregate). 
#' @param aggregation_knnIteration The number of k-nearest neighbor groupings to test for passing the supplied overlapCutoff (number of aimed cell aggregates).
#' @param aggregation_overlapCutoff The maximum allowable overlap between the current group and all previous groups to permit the current group be added to the group list during k-nearest neighbor calculations.
#' @return Co-accessibility map, GRanges object of co-accessibility scores, and tile set of region.
...
plotCoAccessibilityMap <- function(
    ...
    cor_max = NULL,
    aggregation_k = 100,
    aggregation_knnIteration = 500,
    aggregation_overlapCutoff = 0.8
    ){
    ...
    ArchRProj <- addCoAccessibility(ArchRProj, reducedDims = reducedDims, maxDist = 2*(end(region)-start(region)), k = aggregation_k, knnIteration = aggregation_knnIteration, overlapCutoff = aggregation_overlapCutoff)
    ...
    if (is.null(cor_max)){
        cor_max <- max(df_coacc_tiles$correlation)
    }

    palette <- rgb(1, 1 - (0:255)/255, 1 - (0:255)/255)
    
    fields::image.plot(as.matrix(mtx_coacc_tiles), zlim=c(0, cor_max), axes = FALSE, main=as.character(region), col=palette)
    ...
    return(list(CoAccessibilityMap = p, CoAccessibility = coacc_tiles, Tiles = tiles))
}